...
import { cn, formatPersianNumber, normalizePersianText, PriceType } from '@agahiram/shared';
...
const PRICE_TYPE_OPTIONS = [
  { v: PriceType.FIXED, l: 'قیمت مقطوع' },
  { v: PriceType.NEGOTIABLE, l: 'توافقی' },
  { v: PriceType.FREE, l: 'رایگان' },
  { v: PriceType.CALL_FOR_PRICE, l: 'تماس بگیرید' },
] as const;
  ...
  return (
    <Drawer open={open} onOpenChange={onOpenChange}>
      <DrawerContent className="overflow-hidden" style={searchFiltersMaxHeightStyle}>
        ...
        <DrawerBody className="p-0 pb-[var(--composer-stack-keyboard)]">
          {step === 'main' && (
            <MainStep
              local={local}
              setLocal={setLocal}
        ...
        {step === 'main' ? (
          <DrawerFooter className="border-t border-border bg-surface/95 pb-[var(--drawer-input-bottom)] backdrop-blur">
      ...
      <div className="space-y-5 p-4 pb-6">
        ...
        {local.categoryId ? (
          <CategoryAttributeFilters
            ...
            values={local.attributes ?? {}}
            onChange={(attributes) => setLocal((s) => ({ ...s, attributes }))}
          />
        ) : null}
        ...
        {local.cityId ? (
          <NeighborhoodPicker
            cityId={local.cityId}
            valueId={local.neighborhoodId}
            valueName={local.neighborhoodName}
            onPick={(n) =>
              setLocal((s) => ({
                ...s,
                neighborhoodId: n.id,
        ...
        <div className="space-y-2.5">
          ...
          <div className="grid grid-cols-2 gap-2">
            {PRICE_TYPE_OPTIONS.map((o) => (
              <ToggleChip
                key={o.v}
                active={local.priceType === o.v}
                onClick={() =>
                  setLocal((s) => ({
                    ...s,
                    priceType: s.priceType === o.v ? undefined : o.v,
                  }))
                }
              >
                {o.l}
...
function CategoryAttributeFilters({
  categoryId,
  values,
  onChange,
}: {
  ...
  const { data: category, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const r = await apiClient.get<{ attributes?: CategoryAttrDef[] }>(
          ...
          {attr.options.length > 0 ? (
            <div className="flex flex-wrap gap-1.5">
              {attr.options.map((opt) => {
                const active = values[attr.key] === opt;
                return (
                  <ToggleChip
                    key={opt}
                    active={active}
                    onClick={() => {
                      const next = { ...values };
                      if (active) delete next[attr.key];
                      else next[attr.key] = opt;
...
function NeighborhoodPicker({
  ...
  valueId?: string;
  valueName?: string;
  onPick: (n: { id: string; name: string }) => void;
  onClear: () => void;
}) {
      ...
      <div className="flex flex-wrap gap-1.5">
        {neighborhoods.map((n) => (
          <ToggleChip
            key={n.id}
            active={valueId === n.id}
            onClick={() => (valueId === n.id ? onClear() : onPick(n))}
          >
            {n.name}
          </ToggleChip>
        ))}
      </div>
      {valueName ? <p className="text-xs text-muted-foreground">انتخاب‌شده: {valueName}</p> : null}
    </div>
  );
}
